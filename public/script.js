...
document.addEventListener('DOMContentLoaded', function () {
  ...
  axios.get('http://localhost:3000/ExpenseTracker')
      .then((response) => {
  ...
  form.addEventListener('submit', function (event) {
      ...
      axios.post('http://localhost:3000/ExpenseTracker', userDetails)
          .then((result) => {
...
function showAppointmentDetails(userDetails) {
  ...
  deleteBtn.addEventListener('click', function () {
      axios.delete(`http://localhost:3000/ExpenseTracker/${userDetails.id}`)
          .then(() => {
              ul.removeChild(li);
          })
          .catch((error) => {
  ...
  editSubmitBtn.addEventListener('click', function (event) {
      ...
      liText.textContent = `On ${updatedUserDetails.date}, we paid ${updatedUserDetails.amount} for ${updatedUserDetails.type} on ${updatedUserDetails.name}`;

      axios.put(`http://localhost:3000/ExpenseTracker/${updatedUserDetails.id}`, updatedUserDetails)
          .then(() => {
              editForm.style.display = 'none';
          })
          .catch((error) => {
